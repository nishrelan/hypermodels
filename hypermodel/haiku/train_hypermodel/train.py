...
from hypermodel.haiku.loaders import VariableLengthDataset, NumpyDataset
import hydra
...
def main(config):
    ...
    trained_params = train(hypermodel, optimizer, opt_state, hypermodel_params,
                           (x_train_encoded, y_train), num_epochs=config.train.num_epochs,
    ...
    all_x = all_x[idxs]
    all_y = all_y[idxs]
    all_preds = hypermodel.apply(trained_params, encoding(all_x))
